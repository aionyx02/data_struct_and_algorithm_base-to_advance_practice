...
void registerLinearGenerators(CaseGeneratorRegistry& registry) {
    registry.emplace("F01-array-adt", generateArrayAdt);
    registry.emplace("F02-dynamic-array", generateDynamicArray);
    registry.emplace("F03-stack-array", generateStack);
    registry.emplace("F04-circular-queue", generateCircularQueue);
    registry.emplace("F05-array-deque", generateArrayDeque);
    registry.emplace("F06-singly-linked-list", generateSinglyLinkedList);
    registry.emplace("F07-linked-stack", generateLinkedStack);
    registry.emplace("F08-linked-queue", generateLinkedQueue);
    registry.emplace("F09-doubly-linked-list", generateDoublyLinkedList);
    registry.emplace("F10-circular-linked-list", generateCircularLinkedList);
    registry.emplace("F11-singly-list-reverse", generateSinglyListReverse);
    registry.emplace("F12-singly-list-concatenate", generateSinglyListConcatenate);
    registry.emplace("F13-singly-list-split", generateSinglyListSplit);
    registry.emplace("F14-doubly-list-splice", generateDoublyListSplice);
    registry.emplace("F15-fixed-node-pool-list", generateFixedNodePoolList);
    registry.emplace("F16-sentinel-doubly-list", generateSentinelDoublyList);
    registry.emplace("F17-cursor-doubly-list", generateCursorDoublyList);
    registry.emplace("F18-record-table", generateRecordTable);
    registry.emplace("F19-sparse-polynomial", generateSparsePolynomial);
    registry.emplace("F20-triplet-sparse-matrix", generateTripletSparseMatrix);
    registry.emplace("F21-csr-sparse-matrix", generateCsrSparseMatrix);
    registry.emplace("F22-row-major-matrix", generateRowMajorMatrix);
    registry.emplace("F23-row-major-tensor", generateRowMajorTensor);
    registry.emplace("F24-two-stacks-one-array", generateTwoStacksOneArray);
    registry.emplace("F25-circular-linked-queue", generateCircularLinkedQueue);
    registry.emplace("F26-linked-deque", generateLinkedDeque);
}
