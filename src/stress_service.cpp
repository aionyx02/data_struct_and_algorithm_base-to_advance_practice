...
GeneratedCase generateCase(
    ...
    if (problemId == "F52-modulo-hash-function") {
        return generateModuloHashFunction(random, operationCount);
    }
    if (problemId == "F53-linear-probing-hash-table") {
        return generateLinearProbingHashTable(random, operationCount);
    }
    if (problemId == "F54-quadratic-probing-hash-table") {
        return generateQuadraticProbingHashTable(random, operationCount);
    }
    if (problemId == "F55-double-hashing-table") {
        return generateDoubleHashingTable(random, operationCount);
    }
    if (problemId == "F56-tombstone-hash-table") {
        return generateTombstoneHashTable(random, operationCount);
    }
    if (problemId == "F57-node-pool-chained-hash-table") {
        return generateNodePoolChainedHashTable(random, operationCount);
    }
