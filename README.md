...
- Desktop 三欄工作區；題庫與作答欄可拖曳、用方向鍵調寬，並記住個人寬度。
- 一鍵 C++20 編譯或 `Ctrl+Enter`，直接顯示本機 compiler diagnostics。
- 適合窄螢幕的題庫與草稿 drawers。

Web server 只綁定 `127.0.0.1`。編譯功能只使用固定參數執行
`g++ -std=c++20 -fsyntax-only`：不產生或執行 learner binary、不提供 Judge verdict，
也不改寫題目或進度。完成解答後，使用介面右下角提供的
`npm run judge -- <problem-id>` 指令執行完整本地 Judge。
...
| `POST /api/compile` | 對已知題目的 128 KiB 以下草稿執行 bounded syntax-only compile |

所有其他路徑與方法都會拒絕。Web 邊界與後續擴充原則記錄於
[`ADR-0004`](docs/adr/0004-local-read-only-web-adapter.md) 與
[`ADR-0005`](docs/adr/0005-loopback-compile-check.md)。
...
| `npm run web` | 啟動本機 Web 練習工作區與 compile check |
...
| `web/` | 原生 HTML/CSS/ES modules 練習工作區 |
| `scripts/web-server.mjs` | loopback-only 題庫與進度 API、靜態檔案服務 |
