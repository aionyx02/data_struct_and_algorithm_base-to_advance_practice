...
- Priority: P0
- Owner: shawn
- Progress: `A01`-`A05` landed under `problems/advanced/fenwick/` (point update +
  prefix/range, range update / point query, dual range update / range query,
  prefix lower bound, frequency order statistics); `A06`-`A08` remain.
- Related docs:
...
- Acceptance criteria:
  ...
  - [x] Add range update variants, prefix lower bound, and frequency order statistics.
...
- Validation:
  - [x] Known correct submissions for `A01`-`A05` pass deterministic and seeded stress tests.
