...
// Disjoint-set forest that makes amortized cost observable: every find prints
// the number of edges traversed from the queried node to its root, measured
// before path compression flattens the path. Unions are by rank and also
// compress, so a node found twice in a row is far cheaper the second time.

int* parent = nullptr;
int* rankOf = nullptr;

// Returns the root; reports the pre-compression edge count via edges, then
// compresses every node on the walked path directly onto the root.
int findRoot(int value, int& edges) {
    edges = 0;
    int root = value;
