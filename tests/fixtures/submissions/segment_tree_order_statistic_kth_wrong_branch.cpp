...
// Known-wrong: the k-th descend enters the right child WITHOUT subtracting the
// left subtree count from the order, so any k that lands in the right half is
// answered with an inflated rank and returns the wrong key.
...
public:
    ...
    [[nodiscard]] long long less(int bound) const {
        ...
        return rangeCount(1, 0, universe_ - 1, 0, bound - 1);
    }
...
private:
    ...
    [[nodiscard]] int descend(
        ...
        // Bug: forgets `order - leftCount` when moving into the right child.
        return descend(node * 2 + 1, middle + 1, right, order);
    }
