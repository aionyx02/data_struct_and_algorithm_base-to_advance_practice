...
// Known-wrong: reads of individual cells (partial range sums and get) ignore the
// block's lazy add tag, so any cell in a block that carries a pending lazy value
// is reported too small.
...
public:
    ...
    [[nodiscard]] long long get(int index) const {
        // Bug: omits the block's lazy tag.
...
private:
    ...
    [[nodiscard]] long long partialSum(int block, int left, int right) const {
        // Bug: omits the block's lazy tag for the partial cells.
        (void)block;
        long long total = 0;
        for (int index = left; index <= right; ++index) {
