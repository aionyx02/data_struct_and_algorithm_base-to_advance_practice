...
// Known-wrong: reports the correct pre-walk edge count but never compresses, so
// repeated finds of the same deep node keep reporting the same high cost and the
// parent array stays un-flattened.

int* parent = nullptr;
int* rankOf = nullptr;

int findEdges(int value, int& edges) {
    edges = 0;
    int root = value;
...
int main() {
    ...
    for (int operation = 0; operation < operationCount; ++operation) {
        ...
        if (command == "components") {
            ...
            int leftRoot = findEdges(left, ignored);
            int rightRoot = findEdges(right, ignored);
