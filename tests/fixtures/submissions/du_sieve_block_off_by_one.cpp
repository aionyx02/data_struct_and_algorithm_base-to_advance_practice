// Known-wrong fixture for M13: the divisor block in the recurrence counts the
// number of d with quotient q as (r - l) instead of (r - l + 1), so one term per
// block is dropped from both Sphi and Smu whenever the recurrence is used.
#include <iostream>
#include <string>
...
long long sPhi(long long n) {
    ...
    for (long long l = 2; l <= n;) {
        long long q = n / l;
        long long r = n / q;
        res -= (r - l) * sPhi(q);  // BUG: missing +1, drops one term per block.
        l = r + 1;
    }
    phiMemo[n] = res;
    return res;
...
long long sMu(long long n) {
    ...
    for (long long l = 2; l <= n;) {
        long long q = n / l;
        long long r = n / q;
        res -= (r - l) * sMu(q);  // BUG: missing +1, drops one term per block.
        l = r + 1;
    }
    muMemo[n] = res;
    return res;
