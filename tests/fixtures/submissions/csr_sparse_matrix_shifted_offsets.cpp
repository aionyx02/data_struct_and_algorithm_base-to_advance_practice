...
int main() {
    ...
    for (int row = 1; row <= rows; ++row) rowOffsets[row] += rowOffsets[row - 1];
        ...
        if (command == "get") {
            ...
            if (!valid(row, column)) std::cout << "OUT_OF_RANGE\n";
                ...
                for (int index = rowOffsets[row]; index < rowOffsets[row + 1]; ++index) {
                    if (columnIndices[index] == column) result = values[index];
                }
                std::cout << result << '\n';
            }
            ...
            std::cin >> row;
            if (row < 0 || row > rows) std::cout << "OUT_OF_RANGE\n";
            else std::cout << rowOffsets[row] + (row > 0 && row < rows ? 1 : 0) << '\n';
