...
// Known-wrong: coordinates are mapped from maximum down to minimum. Normal
// Fenwick prefixes therefore represent coordinate suffixes.
int main() {
    int minimum = 0;
    int maximum = 0;
    int q = 0;
    ...
    for (int operation = 0; operation < q; ++operation) {
        ...
        if (command == "add") {
            ...
            if (!valid(coordinate)) {
                ...
                values[static_cast<std::size_t>(coordinate - minimum)] += delta;
            ...
            if (!valid(coordinate)) {
                ...
                const int valueIndex = coordinate - minimum;
                const long long delta =
                    value - values[static_cast<std::size_t>(valueIndex)];
                values[static_cast<std::size_t>(valueIndex)] = value;
                update(coordinate, delta);
            }
        } else if (command == "get") {
            int coordinate = 0;
