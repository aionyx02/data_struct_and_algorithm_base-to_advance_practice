...
// Known-wrong: applies the chmin tag on any fully covered node and never
// performs the Beats recursion. When the bound is at or below the second
// maximum, only the maximum elements are lowered while larger second-max
// elements survive, so the sum and the node maximum become wrong.
...
private:
    ...
    void applyRange(
        ...
        // Bug: applies the tag whenever the node is covered, ignoring the
        // strict second maximum, so it never recurses into the Beats step.
