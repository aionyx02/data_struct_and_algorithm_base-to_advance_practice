...
// Known-wrong: a plain BST with subtree sizes but no AVL rebalancing, so order
// statistics stay correct while the reported height exceeds the balanced bound
// on ordered insertions.
...
Node* rebalance(Node* node) {
    update(node);  // bug: never rotates, so the tree is not height-balanced
    return node;
}
