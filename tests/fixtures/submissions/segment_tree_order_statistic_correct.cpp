...
// Count Segment Tree over the value universe [0, universe). Each leaf holds the
// multiplicity of one key; each internal node holds its subtree's total count.
...
public:
    ...
    // One-based k-th smallest key; -1 when order is below 1 or above the size.
