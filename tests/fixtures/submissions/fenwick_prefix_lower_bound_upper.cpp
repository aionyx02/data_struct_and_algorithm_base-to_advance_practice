...
// Known-wrong: the lower bound walk descends while bit[next] <= remaining,
// which finds the first index whose prefix sum is strictly greater than the
// target (an upper bound). On an exact match it returns one index too far,
// or NONE.
int main() {
    int n = 0;
    int q = 0;
