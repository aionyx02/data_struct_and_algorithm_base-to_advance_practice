// Known-wrong fixture for M13: the totient recurrence uses the wrong closed form
// for sum_{d=1}^{n} Sphi(floor(n/d)); it writes n(n-1)/2 instead of n(n+1)/2, so
// every phi_prefix that reaches the recurrence (n above the threshold) is off.
#include <iostream>
#include <string>
...
long long sPhi(long long n) {
    ...
    long long res = n * (n - 1) / 2;  // BUG: should be n(n+1)/2.
