...
// Known-wrong: rollback undoes only the single most recent union instead of
// every union performed since the last checkpoint.
    ...
    for (int operation = 0; operation < operationCount; ++operation) {
        ...
        if (command == "components") {
            ...
            if (checkpointCount == 0) {
                ...
                int undone = 0;
                if (undoCount > target) {  // bug: undoes a single union only
                    --undoCount;
