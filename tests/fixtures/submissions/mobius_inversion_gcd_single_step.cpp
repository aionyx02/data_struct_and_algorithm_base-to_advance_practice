// Known-wrong fixture for M12: the 2D block step only advances by the quotient
// of n, ignoring m. When n != m the m-quotient changes inside a block, so the
// block prefix is multiplied by a stale floor(m/d).
#include <iostream>
#include <string>
#include <vector>
...
long long solve(long long n, long long m, bool useMu) {
    ...
    for (long long l = 1; l <= k;) {
        long long qn = n / l;
        long long qm = m / l;
        long long r = n / qn;  // BUG: ignores m; should be min(n/qn, m/qm).
        if (r > k) r = k;
