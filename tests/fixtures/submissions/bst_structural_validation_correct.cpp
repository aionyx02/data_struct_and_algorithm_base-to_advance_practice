...
bool validateNode(
    ...
    if (state[node->id] != 0 || node->parent != expectedParent ||
        node->key <= low || node->key >= high) {
        return false;
    }
    state[node->id] = 1;
    ++visited;
    if (!validateNode(node->left, node, low, node->key, state, visited) ||
        !validateNode(node->right, node, node->key, high, state, visited)) {
        return false;
    }
    state[node->id] = 2;
    return true;
...
int main() {
    ...
    if (nodeCount == 0) {
        valid = rootId == -1;
    } else if (rootId >= 0 && rootId < nodeCount &&
               root->parent == nullptr) {
        valid = validateNode(
            root,
            nullptr,
