...
int main() {
    ...
    auto printRelative = [&](int index, int relative) {
        if (!valid(index)) std::cout << "OUT_OF_RANGE\n";
        else if (!slots[index].occupied) std::cout << "EMPTY\n";
        else if (!valid(relative) || !slots[relative].occupied) {
            std::cout << "NONE\n";
        } else {
            std::cout << relative << '\n';
        }
    ...
    for (int operation = 0; operation < operationCount; ++operation) {
        ...
        if (command == "set") {
            ...
            if (!valid(index)) {
                std::cout << "OUT_OF_RANGE\n";
            } else if (!slots[index].occupied) {
                std::cout << "EMPTY\n";
            } else {
                const int left = 2 * index + 1;
                const int right = left + 1;
                if ((valid(left) && slots[left].occupied) ||
            ...
            int index = 0;
            std::cin >> index;
            printRelative(index, index == 0 ? -1 : (index - 1) / 2);
        } else if (command == "left_index") {
            int index = 0;
            std::cin >> index;
            printRelative(index, 2 * index + 1);
        } else if (command == "right_index") {
            int index = 0;
            std::cin >> index;
            printRelative(index, 2 * index + 2);
