...
int findPlain(const int* parent, int value) {
    while (parent[value] != value) value = parent[value];
    return value;
}

int findCompressed(int* parent, int value) {
    if (parent[value] != value) {
        parent[value] = findCompressed(parent, parent[value]);
    }
    return parent[value];
}
    ...
    for (int operation = 0; operation < operationCount; ++operation) {
        ...
        if (command == "components") std::cout << components << '\n';
            ...
            if (!valid(value, count)) std::cout << "OUT_OF_RANGE\n";
                ...
                const int root = findCompressed(parent, value);
            ...
            int leftRoot = findCompressed(parent, left);
            int rightRoot = findCompressed(parent, right);
