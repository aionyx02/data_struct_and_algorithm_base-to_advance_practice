// Known-wrong fixture for M11: in the p | i branch phi is multiplied by (p-1)
// instead of p, so phi is wrong on every number whose smallest prime appears
// with exponent >= 2 (prime powers and their multiples).
#include <iostream>
#include <string>
#include <vector>
...
void sieve() {
    ...
    for (int i = 2; i <= LIMIT; ++i) {
        ...
        for (int p : primes) {
            ...
            if (i % p == 0) {
                mu[vv] = 0;
                phi[vv] = phi[i] * (p - 1);  // BUG: should be phi[i] * p.
