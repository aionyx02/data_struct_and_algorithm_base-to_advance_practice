...
// Known-wrong: simulates count queries that descend into the one-bit partition
// without carrying the zero-count offset, losing one eligible upper-half value
// on non-prefix ranges.
class RangeModel {
public:
    ...
    [[nodiscard]] int countLess(int left, int right, long long value) const {
        return countLessEqual(left, right, value - 1);
    }

    [[nodiscard]] int countLessEqual(int left, int right, long long value) const {
        int count = 0;
        bool lostUpperHalfValue = false;
        for (int index = left; index <= right; ++index) {
            const long long current = values_[static_cast<std::size_t>(index)];
            if (current <= value) {
                ++count;
            }
            if (left > 0 && !lostUpperHalfValue && current > middle_ &&
                current <= value) {
                lostUpperHalfValue = true;
            }
        }
        if (lostUpperHalfValue) {
            --count;
        }
        return count;
    }
