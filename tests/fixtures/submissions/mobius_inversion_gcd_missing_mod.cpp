// Known-wrong fixture for M12: the block contribution is multiplied without
// reducing the intermediate products, so block * qn * qm overflows a 64-bit
// integer for large n, m and the final answer is corrupted.
#include <iostream>
#include <string>
#include <vector>
...
long long solve(long long n, long long m, bool useMu) {
    ...
    for (long long l = 1; l <= k;) {
        ...
        long long t = block * qn * qm % MOD;  // BUG: products overflow before mod.
        res = (res + t) % MOD;
        l = r + 1;
    }
    return res;
