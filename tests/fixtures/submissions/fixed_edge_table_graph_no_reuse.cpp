...
int main() {
    ...
    for (int operation = 0; operation < operationCount; ++operation) {
        ...
        if (command == "edge_count") std::cout << edgeCount << '\n';
            ...
            if (!valid(left, vertices) || !valid(right, vertices)) std::cout << "OUT_OF_RANGE\n";
            else if (left == right) std::cout << "SELF_LOOP\n";
            else {
                bool duplicate = findId(edges, capacity, id) >= 0;
                for (int slot = 0; slot < capacity && !duplicate; ++slot) {
                    duplicate = edges[slot].active && samePair(edges[slot], left, right);
                }
                if (duplicate) std::cout << "DUPLICATE\n";
                else if (nextSlot == capacity) std::cout << "FULL\n";
                else {
                    edges[nextSlot] = {id, left, right, true};
                    ++edgeCount;
                    std::cout << nextSlot++ << '\n';
                }
            }
            ...
            else std::cout << edges[slot].id << ' ' << edges[slot].left << ' ' << edges[slot].right << '\n';
            ...
            else std::cout << edges[slot].left << ' ' << edges[slot].right << '\n';
        } else {
            int vertex = -1;
            std::cin >> vertex;
            if (!valid(vertex, vertices)) std::cout << "OUT_OF_RANGE\n";
            else if (command == "degree") {
                int degree = 0;
                for (int slot = 0; slot < capacity; ++slot) {
                    if (edges[slot].active &&
                        (edges[slot].left == vertex || edges[slot].right == vertex)) ++degree;
                }
                std::cout << degree << '\n';
            } else {
                bool first = true;
                for (int neighbor = 0; neighbor < vertices; ++neighbor) {
                    ...
                    for (int slot = 0; slot < capacity && !found; ++slot) {
                        found = edges[slot].active &&
                            ((edges[slot].left == vertex && edges[slot].right == neighbor) ||
                             (edges[slot].right == vertex && edges[slot].left == neighbor));
                    }
                    if (found) {
                        if (!first) std::cout << ' ';
                        std::cout << neighbor;
