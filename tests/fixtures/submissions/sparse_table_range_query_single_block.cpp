...
// Known-wrong: each query consults only the left block [left, left + 2^k - 1]
// and forgets the second overlapping block. Whenever the answer lies in the
// uncovered suffix (right - 2^k, right], the result is wrong.
class SparseTable {
public:
    ...
    [[nodiscard]] long long rangeMin(int left, int right) const {
        ...
        // Bug: only the left block is consulted.
        return minTable_[cell(k, left)];
    }

    [[nodiscard]] long long rangeMax(int left, int right) const {
        const int k = log_[right - left + 1];
        return maxTable_[cell(k, left)];
    }
