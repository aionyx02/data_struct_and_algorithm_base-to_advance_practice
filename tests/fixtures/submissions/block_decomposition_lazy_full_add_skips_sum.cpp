...
// Known-wrong: a range add bumps a whole interior block's lazy tag but forgets
// to add the corresponding amount to that block's cached sum. A later range sum
// that reads the cached sum of such a block is then too small.
...
public:
    ...
    void add(int left, int right, long long delta) {
        ...
        for (int block = leftBlock + 1; block < rightBlock; ++block) {
            // Bug: bumps the lazy tag but forgets to refresh the cached sum.
