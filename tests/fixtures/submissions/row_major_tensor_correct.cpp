...
int main() {
    ...
    auto index = [&](int i, int j, int k) { return (i * d2 + j) * d3 + k; };
    ...
    for (int operation = 0; operation < operationCount; ++operation) {
        ...
        if (command == "set") {
            ...
            std::cin >> i >> j >> k;
            if (!valid(i, j, k)) std::cout << "OUT_OF_RANGE\n";
            else if (command == "get") std::cout << values[index(i, j, k)] << '\n';
            else std::cout << index(i, j, k) << '\n';
