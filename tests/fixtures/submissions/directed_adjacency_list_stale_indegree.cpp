...
bool contains(const int* values, int size, int value) {
    for (int index = 0; index < size; ++index) if (values[index] == value) return true;
    return false;
}
...
int main() {
    ...
    for (int operation = 0; operation < operationCount; ++operation) {
        ...
        if (command == "edge_count") std::cout << edges << '\n';
            ...
            else if (command == "out_degree") std::cout << outDegree[vertex] << '\n';
            else if (command == "in_degree") std::cout << inDegree[vertex] << '\n';
