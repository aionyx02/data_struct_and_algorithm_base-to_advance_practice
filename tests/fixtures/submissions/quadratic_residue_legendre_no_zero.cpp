// Known-wrong fixture for M09: Legendre symbol omits the a=0 case.
...
int main() {
    ...
    for (int i = 0; i < q; ++i) {
        ...
        if (op == "legendre") {
            ll v = a % p;
            if (v < 0) v += p;
            // BUG: omits the a == 0 case, which must print 0.
            std::cout << (powmod(v, (p - 1) / 2, p) == 1 ? 1 : -1) << '\n';
        } else if (op == "is_qr") {
            ll v = a % p;
            if (v < 0) v += p;
