...
int main() {
    ...
    for (int vertex = 0; vertex < count; ++vertex) adjacency[vertex] = new int[count];
    int edges = 0;
    for (int operation = 0; operation < operationCount; ++operation) {
        ...
        if (command == "edge_count") std::cout << edges << '\n';
            ...
            if (!valid(from, count) || !valid(to, count)) std::cout << "OUT_OF_RANGE\n";
                ...
                const bool exists = contains(adjacency[from], degrees[from], to);
                if (command == "has") std::cout << (exists ? "true" : "false") << '\n';
                else if (command == "add") {
                    if (exists) std::cout << "DUPLICATE\n";
                    else if (edges == capacity) std::cout << "FULL\n";
                    else {
                        adjacency[from][degrees[from]++] = to;
                        adjacency[to][degrees[to]++] = from;
                        ++edges;
                    }
