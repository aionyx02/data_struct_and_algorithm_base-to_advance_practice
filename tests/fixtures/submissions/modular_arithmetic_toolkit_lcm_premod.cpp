// Known-wrong fixture for M01: reduces the operands modulo m before computing
// gcd/lcm, which breaks lcm whenever an operand exceeds the modulus.
#include <iostream>
#include <string>
...
int main() {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (op == "gcd") {
            ...
            ll a, b, m;
            std::cin >> a >> b >> m;
            // BUG: reduces operands modulo m before computing gcd and lcm.
            ll A = absll(a) % m;
            ll B = absll(b) % m;
            if (A == 0 || B == 0) {
                std::cout << 0 << '\n';
