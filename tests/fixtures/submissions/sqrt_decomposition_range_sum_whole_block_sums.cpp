...
// Known-wrong: a range sum just adds the cached sums of every block touched by
// the range, ignoring that the two boundary blocks are only partially covered.
// Any query whose endpoints are not block-aligned is wrong.
...
public:
    ...
    [[nodiscard]] long long sum(int left, int right) const {
        // Bug: adds whole block sums for every touched block, ignoring the
        // partially covered boundary blocks.
