...
// Known-wrong: updates mutate the base version's nodes in place, then register
// the same root as a "new" version.
class PersistentSegmentTree {
public:
    PersistentSegmentTree(const long long* values, int size, int operations)
        : size_(size),
          capacity_(static_cast<std::size_t>(size) * 4 +
                    static_cast<std::size_t>(operations + 1) * 8 + 16),
    ...
    int set(int baseVersion, int index, long long value) {
        ...
        roots_[versionCount_] = roots_[baseVersion];
        return versionCount_++;
    }
