...
// Known-wrong: the chmin tag scales the sum reduction by the whole node element
// count instead of the count of maximum elements. Whenever an applied node holds
// fewer maximum elements than its size, the sum is reduced too much.
...
public:
    BeatsSegmentTree(const long long* values, int size)
        : size_(size),
          ...
          nodeCount_(std::make_unique<long long[]>(
              static_cast<std::size_t>(size) * 4
          )),
...
private:
    ...
    void applyTag(int node, long long bound) {
        ...
        // Bug: scales by the node element count instead of the max count.
        sum_[index] -= (maxValue_[index] - bound) * nodeCount_[index];
    ...
    std::unique_ptr<long long[]> nodeCount_;
