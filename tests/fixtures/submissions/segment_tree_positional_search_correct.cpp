...
public:
    ...
    [[nodiscard]] int firstAtLeast(int left, long long threshold) const {
        if (query(left, size_ - 1) < threshold) {
            return -1;
        }
        return findFirst(1, 0, size_ - 1, left, threshold);
    }

    [[nodiscard]] int lastAtLeast(int right, long long threshold) const {
        if (query(0, right) < threshold) {
            return -1;
        }
        return findLast(1, 0, size_ - 1, right, threshold);
    }
...
private:
    ...
    [[nodiscard]] int findFirst(
        ...
        if (right < queryLeft ||
            tree_[static_cast<std::size_t>(node)] < threshold) {
            return -1;
        }
        if (left == right) {
            return left;
        ...
        const int leftAnswer =
            findFirst(node * 2, left, middle, queryLeft, threshold);
        if (leftAnswer != -1) {
            return leftAnswer;
        }
        return findFirst(
    ...
    [[nodiscard]] int findLast(
        ...
        const int rightAnswer =
            findLast(node * 2 + 1, middle + 1, right, queryRight, threshold);
        if (rightAnswer != -1) {
            return rightAnswer;
        }
        return findLast(node * 2, left, middle, queryRight, threshold);
    }
